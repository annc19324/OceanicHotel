    ...
    <main class="container mx-auto px-4 mt-20">
        <div class="content-section">
            ...
            <c:if test="${not empty success}">
                <p class="text-green-600 dark:text-green-400 text-lg text-center mb-4">${success}</p>
            </c:if>
...
<script>
    ...
    function showPaymentOptions(bookingId, totalPrice) {
        ...
        document.getElementById('modalTotalPrice').value = parseFloat(totalPrice.replace(/,/g, '')).toFixed(0);
    ...
    function payWithTest() {
        ...
        fetch('<%= request.getContextPath() %>/user/bookings', {
            ...
            alert('<%= "vi".equals(language) ? "Lỗi khi thực hiện thanh toán Test!" : "Error processing test payment!" %>');
            closePaymentModal();
        });
    }
    ...
    function payWithMoMo() {
        ...
        fetch('<%= request.getContextPath() %>/user/bookings', {
            ...
            if (!response.ok) throw new Error('MoMo payment failed');
            alert('<%= "vi".equals(language) ? "Thanh toán MoMo thành công!" : "MoMo payment successful!" %>');
            closePaymentModal();
            location.reload();
        }).catch(error => {
            console.error('Error:', error);
            alert('<%= "vi".equals(language) ? "Lỗi khi thực hiện thanh toán MoMo!" : "Error processing MoMo payment!" %>');
            closePaymentModal();
        });
    }

    function payWithPayOS() {
        let bookingId = document.getElementById('modalBookingId').value;
        let totalPrice = document.getElementById('modalTotalPrice').value;
        fetch('<%= request.getContextPath() %>/user/bookings', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: 'action=createPayOSLink&bookingId=' + encodeURIComponent(bookingId) + '&totalPrice=' + encodeURIComponent(totalPrice)
        })
        .then(response => {
            if (!response.ok) {
